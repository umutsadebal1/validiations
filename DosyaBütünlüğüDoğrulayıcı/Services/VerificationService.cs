            ...
            try
            {
                ...
                foreach (var hash in allHashes)
                {
                    try
                    {
                        ...
                        {
                            ProgressUpdated?.Invoke($"Atlanıyor: {Path.GetFileName(hash.FilePath)} (dosya bulunamadı)");
                            continue;
                        }
                        ...
                        ProgressUpdated?.Invoke($"Doğrulandı: {Path.GetFileName(hash.FilePath)} ({result.Status})");
                    }
                    catch (Exception ex)
                    {
                        ProgressUpdated?.Invoke($"Hata: {Path.GetFileName(hash.FilePath)} - {ex.Message}");
                    }
                }
